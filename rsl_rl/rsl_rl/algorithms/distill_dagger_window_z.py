...
def build_student(env, train_cfg, device):
    ...
    actor_backbone = FiLMNetwork(
        ...
        backbone=BodyGraphBackbone(obs_dim=proprio_dim + action_dim + phase_dim, d_model=d_model, n_heads=8)
    )
...
def train_step(student, optimizer_student, rb, cfg,
    ...
    for _ in range(cfg.epochs_per_iter):
        for i, (obs_b, mu_t, std_t, v_t, ctxkey_b, phase_b) in enumerate(rb.sample_batches(cfg.batch_size)):
            ...
            with autocast(enabled=use_mixed_precision, dtype=amp_dtype):
                ...
                ctx_mb   = ctx_cpu.to(obs_b.device, non_blocking=True) # [B,C,K]
                ...
                obs_student = torch.cat([base, phase_b, mu_q], dim=-1)

                # 3) 学生前向 & 行为蒸馏
                student.update_distribution(obs_student) # .detach())
