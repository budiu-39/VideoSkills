...
class MotionLibBase():
    ...
    # add by junyu to speed up evaluation
    def sort_loaded_motions_by_length(self, field="pose_aa", descending=False):
        """
        Sort self._motion_data_list and self._motion_data_keys in-place based on length of motion[field].

        Args:
            field (str): name of the key to determine motion length, e.g., "pose_aa" or "smpl_joints"
            descending (bool): if True, sort from long to short. Default: False.
        """
        lengths = np.array([len(m[field]) for m in self._motion_data_list])
        sorted_indices = np.argsort(lengths)
        if descending:
            sorted_indices = sorted_indices[::-1]

        self._motion_data_keys = self._motion_data_keys[sorted_indices]
        self._motion_data_list = self._motion_data_list[sorted_indices]