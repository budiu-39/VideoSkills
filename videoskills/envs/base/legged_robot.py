...
class LeggedRobot(BaseTask):
    ...
    #     # asset_options.enable_dof_force_sensors = True
    ...
    #         actor_handle = self.gym.create_actor(env_handle, robot_asset, start_pose, self.cfg.asset.name, i, -1, 0)
    ...
    def _parse_cfg(self, cfg):
        ...
        # why there is scale for obs? to unify the observation scales with the same clip function?
        self.obs_scales = self.cfg.normalization.obs_scales
        # 21 types of rewards
        self.reward_scales = class_to_dict(self.cfg.rewards.scales)
        # It can accept command? like rotation velocity, linear velocity, heading
        self.command_ranges = class_to_dict(self.cfg.commands.ranges)

        # 20 seconds and 1000 frames per episode
        self.max_episode_length_s = self.cfg.env.episode_length_s
        self.max_episode_length = np.ceil(self.max_episode_length_s / self.dt)
        # seems like external force
