...
def camera_callback(pyrender_viewer, mesh_viewer):
    pose = mesh_viewer.viewer._trackball.pose.copy()
    np.save("scripts/render/camera_pos.npy", pose)
...
class MeshViewer(object):
    ...
    def __init__(self, width=1200, height=800, use_offscreen=False, follow_camera=False, camera_intrinsics=None,
        ...
        if default_cam_rot is None:
            self.default_cam_pose = trimesh.transformations.rotation_matrix(np.radians(0), (0, 0, 1))
            self.default_cam_pose = np.dot(trimesh.transformations.rotation_matrix(np.radians(90), (1, 0, 0)), self.default_cam_pose)
            self.default_cam_pose = np.dot(trimesh.transformations.rotation_matrix(np.radians(20), (0, 0, 1)),
        ...
        registered_keys['c'] = (camera_callback, [self])
    ...
    def animate(self, fps=30):
        ...
        while self.use_offscreen or self.viewer.is_active:
            # cam_node = self.scene.get_nodes(name='pc-camera')
            # self.camera_node = self.scene.add(pc, pose=camera_pose, name='pc-camera')
            #
            # light = pyrender.DirectionalLight(color=np.ones(3), intensity=1.0)
            # self.scene.add(light, pose=self.default_cam_pose)
            ...
            # cam_node = cam_node[0]
            # if not self.use_offscreen:
                # pose = self.viewer._trackball.pose.copy()
                # print(pose)
